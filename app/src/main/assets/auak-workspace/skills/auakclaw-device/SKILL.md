---
name: auak agenty-device
description: "Control your Android phone via auak agenty — screenshot, tap, swipe, type, open apps, execute shell commands, make/accept/end calls, and control audio channels. Use this skill whenever you need to interact with the phone's screen or perform device operations."
metadata:
  openclaw:
    always: true
    emoji: "=^._.^="
...
# auak agenty Device Control

You have direct control over an Android phone through the auak agenty system-level protocol. This is real device control — equivalent to physical finger touches, undetectable by any application.
...
auak agenty provides an HTTP Bridge Server at `http://127.0.0.1:18790`. All device operations go through this bridge via `$PREFIX/bin/curl`.
...
- **HTTP 503**: auak agenty Socket not connected — wait and retry
- **HTTP 500**: Internal error — check the error message in response body
- **Bridge unreachable**: The auak agenty app may not be running or Bridge Server not started

When you get a 503, tell the user that the auak agenty device connection is not active and they may need to check agentyGui Server status.
