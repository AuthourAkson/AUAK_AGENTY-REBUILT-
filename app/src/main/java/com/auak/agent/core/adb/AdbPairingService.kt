...
class AdbPairingService : Service() {
    ...
    companion object {
        const val NOTIFICATION_CHANNEL = "adb_pairing_auak"
        const val ACTION_PAIRING_RESULT = "com.auak.agent.ADB_PAIRING_RESULT"
    ...
    private fun onInput(code: String, port: Int): Notification {
        serviceScope.launch {
            val host = "127.0.0.1"
            val key = try {
                AdbKey(PreferenceAdbKeyStore(getSharedPreferences(ADB_PREFS, MODE_PRIVATE)), "auak")
            } catch (e: Throwable) {
                e.printStackTrace()
                return@launch
            }
