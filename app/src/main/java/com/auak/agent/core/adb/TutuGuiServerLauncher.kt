...
private const val TAG = "agentyGuiServerLauncher"

class agentyGuiServerLauncher(private val context: Context) {

    companion object {
        const val DEVICE_PATH = "/data/local/tmp/scrcpy-server.jar"
        const val SERVER_CLASS = "com.agenty.guiserver.Server"
    ...
    private suspend fun verifyServerProcess(
        ...
        val logContent = try {
            execShell(host, port, key,
                "logcat -d -t 30 -s agenty AndroidRuntime 2>/dev/null || echo 'no logs'", onLog)
        } catch (_: Exception) { "" }
