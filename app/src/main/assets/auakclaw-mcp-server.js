#!/usr/bin/env node
/**
 * auak agenty MCP Server
 * Bridges OpenClaw AI Agent with auak agenty's device control capabilities
 * via auak agenty HTTP Bridge Server on 127.0.0.1:18790
 */

const http = require('http');
const readline = require('readline');

const BRIDGE_HOST = process.env.auak agenty_BRIDGE_HOST || '127.0.0.1';
const BRIDGE_PORT = parseInt(process.env.auak agenty_BRIDGE_PORT || '18790');
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode === 503) {
            reject(new Error(json.error || 'AUAK Agenty Socket not connected'));
...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Check Agenty Bridge and Socket connection status',
    inputSchema: {
      type: 'object',
      properties: {}
    }
...
async function handleToolCall(name, args) {
  switch (name) {
    case 'check_connection': {
      ...
      return [{
        type: 'text',
        text: `Agenty connection: ${status.connected ? 'Connected' : 'Disconnected'} (${status.status}), screen: ${status.screenWidth}x${status.screenHeight}`
      }];
    }
    case 'screenshot': {
  ...
  switch (method) {
    case 'initialize':
      sendMcpResponse(id, {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'Agenty-device', version: '2.0.0' }
      });
      break;

    case 'notifications/initialized': {
      const connected = await checkBridgeAvailable();
      process.stderr.write(`[Agenty-mcp] Bridge ${connected ? 'connected' : 'not available'} at ${BRIDGE_BASE}\n`);
      break;
    }
...
rl.on('line', async (line) => {
  try {
    ...
    process.stderr.write(`[auak agenty-mcp] Error: ${e.message}\n`);
  }
});
