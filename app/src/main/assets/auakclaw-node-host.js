#!/usr/bin/env node
/**
 * auak agenty OpenClaw Node Host
 *
 * Connects to the OpenClaw Gateway via WebSocket and advertises
 * auak agenty device control commands as native Node tools.
 * Routes node.invoke requests to the auak agenty HTTP Bridge (:18790).
 */
...
const IDENTITY_PATH = process.env.auak agenty_NODE_IDENTITY
  || path.join(process.env.HOME || '/data/data/com.auak.agent/files/home', '.openclaw', 'auak agenty-node.json');
const PROTOCOL_VERSION = 3;
const NODE_ID = 'auak agenty';
...
// ── Main ─────────────────────────────────────────────────────────────
function main() {
  log('auak agenty Node Host starting...');
