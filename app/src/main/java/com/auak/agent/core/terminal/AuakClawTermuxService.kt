...
class AuakClawTermuxService(private val context: Context) {
    ...
    fun createSession(
        ...
        if (executable == null) {
            ...
            session.write(". \$HOME/.auak agenty/init.sh\n")
        }
    ...
    private fun writeInitRc() {
        ...
        val script = """
            ...
            cd "${'$'}HOME"
            echo ''
            echo '  =^._.^= AUAK Terminal'
            echo ''
            if [ -x '$prefix/bin/bash' ]; then
                export SHELL='$prefix/bin/bash'
                exec '$prefix/bin/bash' --norc --noprofile -c 'export PS1="AuakClaw\\\$ "; exec bash'
            fi
            export PS1='AuakClaw$ '
        """.trimIndent() + "\n"
    ...
    companion object {
        private const val TAG = "AuakClawTermuxService"
    }
}

class AuakClawTerminalViewClient : TerminalViewClient {
...
class AuakClawTerminalSessionClient(private val context: Context) : TerminalSessionClient {
