...
class LocalSkillRepository(private val dao: SkillDao) {
    ...
    suspend fun importFromMarket(detail: AuakClawSkillDetail): Long {
        return importSkill(detail, LocalSkillEntity.SOURCE_MARKET)
    }

    suspend fun importFromClipboard(detail: AuakClawSkillDetail): Long {
    ...
    fun toAuakClawSkillDetail(entity: LocalSkillEntity): AuakClawSkillDetail {
        return try {
            json.decodeFromString<AuakClawSkillDetail>(entity.rawJson)
        } catch (_: Exception) {
            AuakClawSkillDetail(
